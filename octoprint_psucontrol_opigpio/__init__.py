...
__author__ = "Simon Malpas<simalpas@gmail.com>"
__license__ = (
    "GNU Affero General Public License" + "http://www.gnu.org/licenses/agpl.html"
)
__copyright__ = (
    "Copyright (C) 2024 Simon Malpas "
    + "- Released under terms of the AGPLv3 License"
    + "original author Shawn Bruce<kantlivelong@gmail.com>"
)

import octoprint.plugin


class PSUControl_RPiGPIO(
    ...
    def configure_gpio(self):
        ...
        if self.config["senseGPIOPin"] > 0:
            self._logger.info(
                "Configuring sensing GPIO for pin {}".format(
            ...
            try:
                GPIO.setup(
                    self._gpio_get_pin(self.config["senseGPIOPin"]),
                    GPIO.IN,
                ...
                self._logger.exception(
                    "Exception while setting up GPIO pin {}".format(
                        self.config["senseGPIOPin"]
                    )
                )
        ...
        if self.config["onoffGPIOPin"] > 0:
            self._logger.info(
                "Configuring switching GPIO for pin {}".format(
            ...
            try:
                ...
                GPIO.setup(
                    self._gpio_get_pin(self.config["onoffGPIOPin"]),
                    GPIO.OUT,
    ...
    def turn_psu_on(self):
        ...
        self._logger.debug(
            "Switching PSU On Using GPIO: {}".format(self.config["onoffGPIOPin"])
        )
        if not self.config["invertonoffGPIOPin"]:
            o = GPIO.HIGH
        else:
    ...
    def turn_psu_off(self):
        ...
        self._logger.debug(
            "Switching PSU Off Using GPIO: {}".format(self.config["onoffGPIOPin"])
        )
...
def __plugin_load__():
    ...
    __plugin_hooks__ = {
        "octoprint.plugin.softwareupdate.check_config": __plugin_implementation__.get_update_information  # noqa
    }
